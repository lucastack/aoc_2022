		...
		if (strncmp(line, "move", 4) != 0) {
			...
			if (sscanf(line, "move %d from %d to %d", &amount, &src_index, &dst_index) == 3) {
				...
				for(int i = 0; i < amount; i++){
					...
					int src_pos = src_height - amount + i;
					char char_to_move = real_grid[src_index][src_pos];			
					printf("moving %c from src_index: %d\n", char_to_move, src_pos);
					real_grid[dst_index][dst_height] = char_to_move; 
					real_grid[src_index][src_pos] = '\0';
					heights[dst_index]++;
					for(int i = 0; i < 9; i++){
				...
				heights[src_index] = heights[src_index] - amount;
    			} else {
